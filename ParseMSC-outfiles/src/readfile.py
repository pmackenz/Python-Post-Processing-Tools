...
class OutFile(object):
    ...
    def __init__(self, filename, diameter=10000.0, skin_depth=100000.0):
        ...
        for elemID in self.elements.keys():
            
            if self.elements[elemID].has_no_gpts():
                # original version: get rid of it
                #del self.elements[elemID]
                #continue
                
                # find nodal coordinate list# create cross referencing information for nodes
                nodelist  = []
                for nodeID in self.elements[elemID].getNodes():
                    nodelist.append(self.nodes[nodeID].getPos())

                # initialize Gauss Point position
                self.elements[elemID].initGaussPoints(array(nodelist), self.x0)
        ...
        if self.file_open:
            for self.line in self.file:
                if (self.line.find('s t a r t   o f   i n c r e m e n t') > 0):
                    ...
                    if (self.increments.has_key(nInc)):
    ...
    def scanIncrementZero(self):
        keywords = ('coordinates','connectivity','end option')
        
        if self.file_open:
            for self.line in self.file:
                #if (self.line.find('e n d   o f   i n c r e m e n t') >= 0):
                if (self.line.find('end option') == 13):
                    return 0
    ...
    def getGPinfo(self):
        ...
        if (self.elements.has_key(elmtId)):
    ...
    def verifyIncrementData(self, inc=-1):
        if inc < 0:
            ...
            if self.increments.has_key(inc):
    ...
    def InctoTime(self,inc):
        t = (inc - (self.nskip+1))*15
        hour = t // 60
        mins = t % 60
    
        time1 = "{:02d}{:02d}".format(hour, mins)
        time2 = "{:02d}:{:02d}".format(hour,mins)
        return [time1,time2]
